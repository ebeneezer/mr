...
struct MRVMStagedExecutionContext {
	...
	void initializeSession() {
		mrvmInitializeBackgroundEditSession(session, input);
	}

	MRMacroStagedJobResult buildResult(const VirtualMachine &vm) const {
		return mrvmBuildStagedJobResult(vm, session);
	}
};
}
...
void mrvmInitializeBackgroundEditSession(BackgroundEditSession &session, const MRMacroStagedExecutionInput &input) {
	...
	for (const auto &globalInt : input.globalInts) {
		GlobalEntry entry;
		const std::string key = mrvmUpperKey(globalInt.first);
	...
	for (const auto &globalString : input.globalStrings) {
		GlobalEntry entry;
		const std::string key = mrvmUpperKey(globalString.first);
	...
	for (const auto &macroDisplayName : input.macroDisplayNames) {
		const std::string key = mrvmUpperKey(macroDisplayName.first);
...
MRMacroStagedJobResult mrvmBuildStagedJobResult(const VirtualMachine &vm, const BackgroundEditSession &session) {
	...
	for (const std::string &key : session.globalOrder) {
		const std::map<std::string, GlobalEntry>::const_iterator it = session.globals.find(key);
