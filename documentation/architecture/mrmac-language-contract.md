...
For closure and execution-session variables, mutation of the paused VM value
and publication into the central K/V store is one semantic transaction under
the VM execution mutex. Failure leaves both representations unchanged.
...
The stable source identity is `normalized-resolved-path^macro-name`. Internal
ownership, breakpoint rebinding and the one-debugger-per-source rule use this
complete identity. The UI renders only the macro name.
