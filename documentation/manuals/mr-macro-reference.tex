...
line number in the source view's visible line-number gutter. The gutter must
be visible and debugger controls active. This invokes the same action as F9
and resolves folded viewport rows to document lines; it is not a folding-gutter
gesture. The same mouse action is available in the compiled-program GDB
debugger, with that backend's source-location rules.
