...
\primitiveoverview{MESSAGES}{variable-messages}{variable}{Hero message-line switch.}
...
PRINT_MARGIN
\end{verbatim}
Parameters: None. Result: read-only integer value.

Returns the configured right boundary for PDF text output in columns.  The
value applies when the current file or a marked block is exported.  A value of
zero disables the column limit, leaving the printable page width as the only
horizontal boundary.  Assigning to this variable is a runtime error.
...
Reads the Hero message-line switch: one means enabled and zero means disabled.
Assigning zero disables that destination; assigning a nonzero integer enables
it. This does not mute ordinary warnings or validation feedback, change Hero
log output, or alter the file-size threshold. The central settings model
records a dirty change only when the value actually changes.

Do not confuse this runtime variable with the retired serialized setting of
the same name. The old settings key is accepted without changing runtime
state; current settings use the Hero keys described under
\hyperref[prim:procedure-mrsetup]{MRSETUP}.
...
$MACRO REFERENCE_VARIABLE_MESSAGES FROM EDIT;
    ...
    Value := MESSAGES;
    MESSAGES := 0;
    MAKE_MESSAGE('Ordinary feedback remains available.');
    MESSAGES := Value;
END_MACRO;
\end{verbatim}
\end{mrmacextension}
...
Indents the visible committed block through the editor's block command route.
A stream block is first converted to a line block covering its touched lines.
For line blocks, the least-indented nonblank line determines the delta to the
next configured tab stop; every nonblank line receives that same delta, so
relative indentation is preserved. Whitespace-only lines are unchanged.
Column blocks retain their rectangular shift semantics. The document must be
writable and a visible block must exist. The text change is undoable.
...
Undents the visible block towards the previous configured tab stop. Stream
blocks are first converted to line blocks covering the touched lines. The
least-indented nonblank line determines one shared reduction for a line block;
relative indentation is preserved and whitespace-only lines are skipped.
Column blocks retain their rectangular shift semantics. A missing visible
block or read-only document prevents the mutation. The text change is
undoable.
...
\primitiveoverview{DESKTOP\_BLIT}{procedure-desktop-blit}{procedure}{MR desktop background rendering.}
\primitiveoverview{DESKTOP\_CLEAR}{procedure-desktop-clear}{procedure}{MR desktop background rendering.}
\primitiveoverview{DESKTOP\_PUT\_CHAR}{procedure-desktop-put-char}{procedure}{MR desktop background rendering.}
\primitiveoverview{DESKTOP\_PUT\_STRING}{procedure-desktop-put-string}{procedure}{MR desktop background rendering.}
\primitiveoverview{DESKTOP\_SET\_COLOR}{procedure-desktop-set-color}{procedure}{MR desktop background rendering.}
...
\begin{mrmacextension}
\phantomsection
\label{prim:procedure-desktop-blit}
\subsection*{DESKTOP\_BLIT}
\index{DESKTOP\_BLIT@\texttt{DESKTOP\_BLIT}}
\begin{verbatim}
DESKTOP_BLIT(path, column, row, mode);
\end{verbatim}
Parameters:
\begin{description}
\item[path] Path of a binary-read UTF-8 ANSI image file.
\item[column] One-based column in the MR desktop background.
\item[row] One-based row in the MR desktop background.
\item[mode] Rendering mode: 0 for direct ANSI colour or 1 for desktop-adaptive
monochrome.
\end{description}

Reads and renders an ANSI image without an intermediate colour-array or image
conversion. Standard ANSI foreground colours 30 through 37 and 90 through 97,
and background colours 40 through 47 and 100 through 107, are mapped directly
to their equivalent TVision/BIOS colour nibbles in mode 0. Reset, default
colour, bold foreground and inverse-video SGR parameters are supported.

Mode 1 maps all 16 decoded ANSI colours to distinct, fixed,
luminance-ordered monochrome ranks. Equal upper/lower block samples use the
CP437 \texttt{light-shade}, \texttt{medium-shade}, \texttt{dark-shade} and
\texttt{full-block} progression. A deterministic 4 by 4 ordered dither selects
between adjacent progression levels. Unequal upper/lower samples remain two
independent luminance planes: one common ordered threshold selects the
light-shade, upper-half, lower-half or full-block glyph, so the two samples are
never averaged. Ordinary non-block glyphs remain unchanged. The configured
desktop attribute supplies the only output colour pair, and minimum luminance
reproduces the native desktop shade instead of introducing a solid rectangular
background. The conversion is fixed and does not depend on image histograms,
placement or clipping.

The parser accepts CR, LF, backspace, tab, UTF-8 text representable in the
active TVision code page, CSI cursor positioning and movement
(\texttt{H}, \texttt{f}, \texttt{A} through \texttt{D}, \texttt{G},
\texttt{d}), cursor save/restore, full erase \texttt{2J}, and cursor visibility
\texttt{?25h}/\texttt{?25l}. LF begins the next image row. Unsupported or
malformed control sequences and unrepresentable characters are reported
instead of being discarded. Input is limited to 16 MiB, dimensions below 4096
cells per axis and at most 1,048,576 image cells.

The decoded rectangle is clipped to the desktop background. The operation does
not change the current desktop drawing attribute. It performs external file I/O
and therefore executes on the UI-thread route rather than as a staged
background macro operation. Windows remain above the canvas, and the menu and
FLabel lines are outside its projection bounds. The canvas is volatile runtime
state shared by all virtual desktops.

\paragraph{Example macro.}
\begin{verbatim}
$MACRO REFERENCE_DESKTOP_BLIT FROM EDIT;
    DESKTOP_CLEAR();
    DESKTOP_BLIT('misc/example.ans', 1, 1, 0);
END_MACRO;
\end{verbatim}
\end{mrmacextension}
...
\begin{mrmacextension}
\phantomsection
\label{prim:procedure-desktop-clear}
\subsection*{DESKTOP\_CLEAR}
\index{DESKTOP\_CLEAR@\texttt{DESKTOP\_CLEAR}}
\begin{verbatim}
DESKTOP_CLEAR();
\end{verbatim}
Parameters: None.

Removes all retained MRMAC canvas cells and exposes the native MR desktop
background again. The current desktop drawing attribute is preserved, so a
subsequent character or string operation continues with the attribute selected
before the clear. Windows and the virtual-desktop marker remain above the
canvas; the menu and FLabel lines remain outside its projection bounds.

\paragraph{Example macro.}
\begin{verbatim}
$MACRO REFERENCE_DESKTOP_CLEAR FROM EDIT;
    DESKTOP_SET_COLOR(31);
    DESKTOP_PUT_STRING('temporary text', 1, 1);
    DESKTOP_CLEAR();
    DESKTOP_PUT_STRING('attribute retained', 1, 1);
END_MACRO;
\end{verbatim}
\end{mrmacextension}
...
\begin{mrmacextension}
\phantomsection
\label{prim:procedure-desktop-put-char}
\subsection*{DESKTOP\_PUT\_CHAR}
\index{DESKTOP\_PUT\_CHAR@\texttt{DESKTOP\_PUT\_CHAR}}
\begin{verbatim}
DESKTOP_PUT_CHAR(character, column, row);
\end{verbatim}
Parameters:
\begin{description}
\item[character] Character value or a string containing exactly one byte.
\item[column] One-based column in the MR desktop background.
\item[row] One-based row in the MR desktop background.
\end{description}

Writes one character cell using the current attribute selected by
DESKTOP\_SET\_COLOR. An out-of-bounds cell is clipped. The retained
cell is redrawn after expose, resize and virtual-desktop changes without
covering windows, the menu or the FLabel line.\footnote{The historical MEMAC
DRAW\_CHAR procedure is not available in MRMAC.}

\paragraph{Example macro.}
\begin{verbatim}
$MACRO REFERENCE_DESKTOP_PUT_CHAR FROM EDIT;
    DESKTOP_SET_COLOR(30);
    DESKTOP_PUT_CHAR('*', 10, 4);
END_MACRO;
\end{verbatim}
\end{mrmacextension}
...
\begin{mrmacextension}
\phantomsection
\label{prim:procedure-desktop-put-string}
\subsection*{DESKTOP\_PUT\_STRING}
\index{DESKTOP\_PUT\_STRING@\texttt{DESKTOP\_PUT\_STRING}}
\begin{verbatim}
DESKTOP_PUT_STRING(text, column, row);
\end{verbatim}
Parameters:
\begin{description}
\item[text] String-like sequence of character bytes.
\item[column] One-based starting column in the MR desktop background.
\item[row] One-based row in the MR desktop background.
\end{description}

Writes the bytes of text from left to right using the current desktop drawing
attribute. Text is clipped to the desktop background. The retained cells are
shared by all virtual desktops and do not extend into the menu or FLabel line.

\paragraph{Example macro.}
\begin{verbatim}
$MACRO REFERENCE_DESKTOP_PUT_STRING FROM EDIT;
    DESKTOP_SET_COLOR(31);
    DESKTOP_PUT_STRING('MR desktop', 3, 2);
END_MACRO;
\end{verbatim}
\end{mrmacextension}
...
\begin{mrmacextension}
\phantomsection
\label{prim:procedure-desktop-set-color}
\subsection*{DESKTOP\_SET\_COLOR}
\index{DESKTOP\_SET\_COLOR@\texttt{DESKTOP\_SET\_COLOR}}
\begin{verbatim}
DESKTOP_SET_COLOR(attribute);
\end{verbatim}
Parameters:
\begin{description}
\item[attribute] Complete TVision character attribute in the range 0 through
255.
\end{description}

Selects the attribute used by subsequent DESKTOP\_PUT\_CHAR and
DESKTOP\_PUT\_STRING operations in applied command order. The
attribute and desktop cells are volatile runtime state and are not persisted.
DESKTOP\_BLIT resolves an attribute for every cell and does not change this
value.\footnote{The historical MEMAC DRAW\_ATTR procedure is not available in
MRMAC.}

\paragraph{Example macro.}
\begin{verbatim}
$MACRO REFERENCE_DESKTOP_SET_COLOR FROM EDIT;
    DESKTOP_SET_COLOR(31);
    DESKTOP_PUT_STRING('coloured text', 1, 1);
END_MACRO;
\end{verbatim}
\end{mrmacextension}
...
Adds one serialized keymap binding definition for a defined profile. A
binding names its profile, context, type, target, and key sequence. The current
factory WordStar, Emacs, and Nano profiles bind Ctrl+A to the whole-document
line-block action. The former edit-prefixed action remains a compatibility
alias:
\begin{verbatim}
MR_BLOCK_MARK_ALL
MR_EDIT_MARK_ALL
\end{verbatim}
Installed personal keymaps are not silently overwritten by a package update.
The action is not an Emacs/Nano beginning-of-line binding.
\index{MR\_BLOCK\_MARK\_ALL}\index{MR\_EDIT\_MARK\_ALL}

\paragraph{Example macro.}
The WORDSTAR profile must already exist. This changes its Ctrl+A binding.
\begin{verbatim}
$MACRO REFERENCE_KEYMAP_BIND FROM EDIT;
    KEYMAP_BIND('name="WORDSTAR" context=EDIT type=ACTION ' +
                'target="MR_BLOCK_MARK_ALL" sequence="<Ctrl+A>"');
END_MACRO;
\end{verbatim}
\end{mrmacextension}
...
\begin{verbatim}
MRCOMPILERPROFILE(operation, profileId, argument3, argument4);
\end{verbatim}
Parameters (all strings):
\begin{description}
\item[operation] DEFINE to create a profile, or SET to assign one of its settings.
\item[profileId] New profile ID for DEFINE; existing ID for SET.
\item[argument3] Display name for DEFINE; setting key for SET.
\item[argument4] Supported toolchain for DEFINE; serialized setting value for SET.
\end{description}
Duplicate definitions and unknown settings are errors.

This is a startup settings directive. Ordinary edit macros cannot call it.
Compatible compiled-program debugging requires a Debug profile and a current
executable artifact; compiler selection alone does not start GDB. Supported
GDB toolchains are GCC, CLANG, GCC\_C, CLANG\_C, RUST, GO, FPC, and FREEBASIC.

\paragraph{Example settings source.}
These lines illustrate a profile definition within controlled startup source;
retain the surrounding canonical settings and build-version declaration.
\begin{verbatim}
MRCOMPILERPROFILE('DEFINE', 'C_DEBUG_DOC', 'C Debug', 'GCC_C');
MRCOMPILERPROFILE('SET', 'C_DEBUG_DOC', 'EXECUTABLE', '/usr/bin/gcc');
\end{verbatim}
Use Compiler profiles and Automatic Setup to obtain the full build arguments
and output configuration appropriate to the installed toolchain.
\end{mrmacextension}
...
\begin{verbatim}
MRFEPROFILE(operation, profileId, argument3, argument4);
\end{verbatim}
Parameters (all strings):
\begin{description}
\item[operation] DEFINE to create a profile, EXT to add an extension, or SET
 to assign an editor setting.
\item[profileId] New profile ID for DEFINE; existing ID for EXT or SET.
\item[argument3] Display name for DEFINE, filename extension for EXT, or setting
 key for SET.
\item[argument4] Empty for DEFINE and EXT; serialized setting value for SET.
\end{description}
These are editor-profile directives, not source/output file arguments.

The directive is legal only in controlled startup settings source. A matching
filename-extension profile can override the global automatic bracket-pairing
option independently of other editor settings.

\paragraph{Example settings source.}
\begin{verbatim}
MRFEPROFILE('DEFINE', 'C_DOC', 'C source', '');
MRFEPROFILE('EXT', 'C_DOC', 'c', '');
MRFEPROFILE('SET', 'C_DOC', 'AUTO_PAIR_BRACKETS', 'TRUE');
\end{verbatim}
These lines belong in a complete canonical startup settings source, not in an
ordinary edit macro. See the bracket-pairing semantics under
\hyperref[prim:procedure-mrsetup]{MRSETUP}.
\end{mrmacextension}
...
Parameters:
\begin{description}
\item[key] String naming a supported settings key.
\item[value] String containing that key's serialized value. Boolean values
accept the settings parser's boolean literals; numeric settings validate
their own integer range.
\end{description}

Applies an assignment through the central settings model during controlled
startup execution. Ordinary edit macros cannot use it to change arbitrary
settings. The existing non-startup File Compare minimap colour route is a
narrow exception, not general permission to call startup directives.

\paragraph{Hero destinations and threshold.}
\index{HERO\_MESSAGES\_ON\_MESSAGELINE}
\index{HERO\_MESSAGES\_IN\_LOGFILE}
\index{HERO\_MESSAGES\_FILE\_THRESHOLD\_MB}
\begin{verbatim}
HERO_MESSAGES_ON_MESSAGELINE
HERO_MESSAGES_IN_LOGFILE
HERO_MESSAGES_FILE_THRESHOLD_MB
\end{verbatim}
The first two keys independently enable the Hero message-line and log
destinations; defaults are true and false. The third accepts integers 0--100,
default eight, in units of 1,048,576 bytes. File-aware Hero notices require a
file strictly larger than the threshold before either destination is used.
Non-file Hero notices do not use that filter. Ordinary warning and validation
owners are unaffected. The retired serialized MESSAGES key is a no-op;
\hyperref[prim:variable-messages]{the runtime variable} still controls the
Hero message-line destination.

\paragraph{Automatic bracket pairing.}
\index{AUTO\_PAIR\_BRACKETS}
\begin{verbatim}
AUTO_PAIR_BRACKETS
\end{verbatim}
This boolean defaults to false and can also be overridden in a matching
filename-extension profile. With insert mode on, no selection, and a
single-character text event, an opening parenthesis, bracket, or brace inserts
its closer and leaves the cursor between the pair. A matching closer already
at the cursor is skipped. Quotes, bracketed paste, overwrite mode, and
selected-text replacement do not take this route; syntax classification does
not exclude comments or strings.

\paragraph{Example settings source.}
Keep the complete generated settings source and its version declaration;
these are example assignments inside that source, not an edit macro.
\begin{verbatim}
MRSETUP('HERO_MESSAGES_ON_MESSAGELINE', 'TRUE');
MRSETUP('HERO_MESSAGES_IN_LOGFILE', 'FALSE');
MRSETUP('HERO_MESSAGES_FILE_THRESHOLD_MB', '8');
MRSETUP('AUTO_PAIR_BRACKETS', 'TRUE');
\end{verbatim}
\end{mrmacextension}
...
\primitiveoverview{OS\_VERSION}{intrinsic-os-version}{intrinsic}{Shell version query.}
...
\begin{mrmacextension}
\phantomsection
\label{prim:intrinsic-os-version}
\subsection*{OS\_VERSION}
\index{OS\_VERSION@\texttt{OS\_VERSION}}
\begin{verbatim}
OS_VERSION()
\end{verbatim}
Parameters: None.

Invokes the configured shell with its version option through the controlled
subprocess path when evaluated. Returns the first output line as a string. If
the shell produces no output, returns the shell executable name.

\paragraph{Example macro.}
\begin{verbatim}
$MACRO REFERENCE_OS_VERSION FROM EDIT;
    DEF_STR(Result);
    Result := OS_VERSION();
END_MACRO;
\end{verbatim}
...
Moves the cursor to the matching parenthesis, square bracket, or brace. The
origin is the delimiter at the cursor or immediately before it. Matching
counts nesting of that delimiter pair in the document text; strings and
comments are not excluded. Missing delimiters or matches produce feedback
without moving to an invented location.
...
Opens Color setup. Select a role and edit its foreground/background RGB
components with six sliders in the range 0--255. The hexadecimal pair is a
readout; it and the role sample update immediately with each component
change, including during a mouse drag. Role changes retain previous component
edits in the same dialog draft. The sample uses the configured output mode.
OK applies and persists changes; closing a dirty draft uses Save/Discard/Cancel
gating. Load Theme activates a theme and Save Theme applies and writes the
draft. Those explicit operations are not rolled back by later cancellation.
RGB versus palette output is selected in User Interface Settings.
...
Opens User Interface Settings, including independent Hero message-line/log
destinations, the synchronized 0--100 MB threshold slider and editable field,
and Color Management. Color Management selects automatic 24-bit RGB output
or terminal-palette output; the RGB role values remain the same settings in
either mode. Invalid input prevents applying the dialog draft.
...
not create source maps. A debugger session runs real MRMAC semantics and keeps
the macro's normal execution profile. Background-safe code runs in finite
Macro workers, staged code keeps the normal snapshot, conflict and UI-thread
commit path, and remaining UI-affine code advances in bounded UI-pump slices.
File, editor and UI side effects are therefore not a preview, but none of
these routes may monopolise the UI thread.
\end{mrmacextension}
...
\begin{mrmacextension}
\section{Source-view context menu}

A plain single right-click in the debugger source text opens the ordinary
editor context menu. Edit contains Cut, Copy, and Unmark when text is marked,
plus Paste; without marked text it contains Paste. Outline lists available
structure from open source buffers. At a supported text target, References
and Rename open the multi-file search or replacement dialog with that text
prefilled. These operations use text matching, not MRMAC binding identity.
Escape dismisses the menu; Enter selects an item or opens the Edit submenu.
\index{context menu!debugger source}

The extra breakpoint/run/watch/evaluation/step group in the compiled-program
GDB source menu is specific to that backend. Use the MRMAC debugger controls
below for macro execution. The left-button gutter double-click is shared by
both debuggers.

\section{Breakpoints, running and stopping}

\texttt{F9} sets a breakpoint at the cursor source line or clears one already
there. Alternatively, double-click with the left mouse button on the desired
row in the source view's debugger gutter. The gutter is visible while debugger
controls are active. This invokes the same action as F9 and resolves folded
viewport rows to document lines; it is independent of the line-number and
folding gutters. The same mouse action is available in the compiled-program
GDB debugger, with that backend's source-location rules.
\index{breakpoint!gutter double-click}\index{debugger gutter!breakpoints}
The MRMAC debugger maps the line to its debuggable source span, so a comment,
...
and rebound when that workspace is restored. The persisted definition contains
the source and macro identity, source line, enabled state, optional condition
and watch expression. It contains no live session, instruction pointer, call
stack, variable value, generated source map or bytecode offset. Restore
therefore creates a cold debugger with its breakpoints and watches, not a
resurrected paused VM.

Exactly one debugger may own a source identity at a time. Internally that
identity is the normalized resolved source path plus the macro name; the UI
shows the familiar \texttt{Filename\textasciicircum MacroName} form. If a
stored breakpoint cannot bind to a debuggable span after the source is
recompiled, it remains visible with the Debugger group's unbound colour, is
logged, and is retried by a later debug start.
\end{mrmacextension}
...
Intrinsics, external I/O and deferred UI playback are black boxes for stepping
and cooperative pause. The debugger never interrupts one halfway through.
When background or staged execution reaches a breakpoint, pause or step stop,
its finite worker ends and the VM remains parked with the Execution Session;
the paused debugger consumes no worker. Continue or Step starts another finite
worker action. UI-affine execution uses the same stop policy in bounded
event-pump slices.
...
The debugger panes show the scopes that MRMAC actually implements: locals,
application globals, closure variables and execution-session variables.
MRMAC has no file-global scope; a \texttt{DEF\_*} declaration belongs to its
current VM, closure or long-lived session, while an explicit global belongs to
the process-wide application-global store.

While a session is paused, select an existing integer, real, string or
character scalar in Variables to open its inline value field. Enter validates
and writes the new value, then refreshes the complete Variables tree and
Watches; Esc cancels the edit. The VM validates the root name, actual scope,
type and complete collection path again at the write boundary. The pane is not
a writable shadow of VM state.
\end{mrmacextension}
...
\begin{mrmacextension}
Hashes and arrays appear as complete indented trees, including nested
collections and explicit cycle references. A container row still shows its
type and size, but every key or one-based array element follows as its own
selectable row; there is no arbitrary collection snapshot limit.

Selecting a hash container opens an insertion field. Enter
\texttt{key=value} for a string entry, or
\texttt{key:type=value} where \texttt{type} is \texttt{int},
\texttt{real}, \texttt{str}, \texttt{char}, \texttt{hash},
\texttt{int[]}, \texttt{real[]}, \texttt{str[]}, \texttt{char[]} or
\texttt{hash[]}. Selecting an array container appends one element of that
array's declared type. On a non-root element, \texttt{:delete} removes it;
\texttt{:rename=newKey} renames a hash entry. Nested scalar rows use the normal
typed scalar edit. To store a scalar string beginning with one colon, enter
two leading colons. Every successful structural change is applied to the
paused VM's local, closure, session or application-global backing store and is
then projected afresh.
\end{mrmacextension}

\debuggerscreenshot{DBG_multilevel_hashes.png}{fig:debugger-hash}{A nested hash is projected as a typed hierarchy in Variables. Container summaries remain visible while keys and nested values are individually selectable at the paused location.}
