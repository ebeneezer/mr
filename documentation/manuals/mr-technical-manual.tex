...
\begin{figure}[htbp]
  ...
  \begin{picture}(1000,540)
    \put(0,0){\includegraphics[width=0.98\textwidth]{../pngsjpegs/COPRO_warming.png}}
...
Figure~\ref{fig:deferred-scan-windows} shows the common shape without erasing
the domain differences. A viewport demand creates a generation and reserves
only uncovered packet spans. Finite workers may complete out of order and may
remain useful after a cursor jump. The ledger first accepts matching results;
the domain then advances its confirmed frontier or visible window as its
...
Figure~\ref{fig:coprocessor-lanes} shows the complete path from a domain packet
to an adopted result.
...
adoption and ordinary TVision projection. Figure~\ref{fig:minimap-ui}
identifies the three visible components, and
Figure~\ref{fig:minimap-function-flow} follows the complete data path.
...
\begin{figure}[htbp]
  ...
  \begin{picture}(1000,543)
    \put(0,0){\includegraphics[width=0.98\textwidth]{../pngsjpegs/UI_minimap.png}}
...
Figure~\ref{fig:mrmac-session-routes} separates the routes and their lifetimes.
The runner makes the routing decision; the scheduler is concerned with
interval and overlap policy, while the coprocessor owns only the finite worker
