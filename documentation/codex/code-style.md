...
- About 1000 lines is a review guideline against excessive source-file growth, not a hard splitting threshold. Small, bounded fixes do not require a split or an exception approval solely because the file exceeds that size. When continued growth warrants a split, use semantically named subordinate source files along stable domain boundaries, preserve ownership and visibility, and avoid secondary registries, broad façade APIs or helper networks only to move lines.
