...
void Win32ConsoleAdapter::initEncoding(bool isLegacyConsole, UINT &cpInput,
                                       UINT &cpOutput) noexcept {
...
Win32Input::Win32Input(ConsoleCtl &aCon) noexcept : InputAdapter(aCon.in()), con(aCon) {
}
...
Win32Display::Win32Display(ConsoleCtl &aCon, bool isLegacyConsole) noexcept : con(aCon) {
	if (!isLegacyConsole)
...
static bool getWin32KeyText(const KEY_EVENT_RECORD &KeyEvent, TEvent &ev,
...
// in which case we need the next event.
{
	...
	if (' ' <= ch && ch != 0x7F) {
		...
		ev.keyDown.textLength =
		    WideCharToMultiByte(CP_UTF8, 0, utf16, utf16[1] ? 2 : 1, ev.keyDown.text,
		                        sizeof(ev.keyDown.text), nullptr, nullptr);
#else
		(void)state;
...
bool getWin32Key(const KEY_EVENT_RECORD &KeyEvent, TEvent &ev, InputState &state) noexcept {
	...
	ev.keyDown.controlKeyState =
	    KeyEvent.dwControlKeyState & (kbShift | kbCtrlShift | kbAltShift | kbScrollState |
	                                  kbNumState | kbCapsState | kbEnhanced);
	...
	if (ev.keyDown.keyCode == 0x2A00 || ev.keyDown.keyCode == 0x1D00 ||
		...
		ev.keyDown.keyCode = kbNoKey;
	         ...
	         (ev.keyDown.controlKeyState & kbRightAlt) && ev.keyDown.textLength == 0)
		...
		ev.keyDown.keyCode = kbNoKey;
	         ...
	         (ev.keyDown.controlKeyState & kbAltShift) && ev.keyDown.textLength != 0)
...
void getWin32Mouse(const MOUSE_EVENT_RECORD &MouseEvent, TEvent &ev, InputState &state) noexcept {
	...
	ev.mouse.controlKeyState =
	    MouseEvent.dwControlKeyState & (kbShift | kbCtrlShift | kbAltShift | kbScrollState |
	                                    kbNumState | kbCapsState | kbEnhanced);
