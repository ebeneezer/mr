...
// Request IDs
const char f2lNoAnswer = '\0', f2lClipGetData = '\xA0';
		...
		if (out.back() == 'K' && out.size() - 1 == 14) {
			...
			if (uint16_t keyCode = virtualKeyCodeToKeyCode[kev.wVirtualKeyCode]) {
				...
				if ((kev.dwControlKeyState & (kbCtrlShift | kbAltShift)) &&
				    L' ' <= kev.uChar.UnicodeChar && L'\x7F' != kev.uChar.UnicodeChar)
...
template <bool write = true, class... Args> inline size_t concat(char *) noexcept {
	return 0;
}
