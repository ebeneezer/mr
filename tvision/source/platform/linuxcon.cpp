...
inline LinuxConsoleAdapter::LinuxConsoleAdapter(DisplayAdapter &aDisplay,
                                                ...
                                                InputState &aInputState, SigwinchHandler *aSigwinch,
                                                GpmInput *aGpm) noexcept
    : ConsoleAdapter(aDisplay, {&aWrapper, aGpm, aSigwinch}), inputState(aInputState),
      sigwinch(aSigwinch), wrapper(aWrapper), gpm(aGpm) {
...
LinuxConsoleAdapter &LinuxConsoleAdapter::create(ConsoleCtl &con, DisplayBuffer &displayBuf,
...
ushort LinuxConsoleInput::convertLinuxKeyModifiers(ushort linuxShiftState) noexcept {
	constexpr struct {
	    ...
	    {1 << KG_SHIFT, kbShift}, {1 << KG_CTRLL, kbLeftCtrl}, {1 << KG_CTRLR, kbRightCtrl},
	    {1 << KG_ALT, kbLeftAlt}, {1 << KG_ALTGR, kbRightAlt},
	};
