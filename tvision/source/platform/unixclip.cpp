...
constexpr const char *wslPasteArgv[] = {
    "/mnt/c/Windows/System32/cmd.exe", "/D", "/Q", "/C", wslPasteCmd, 0};
...
static run_subprocess_t run_subprocess(const char *const argv[], const char *const env[],
                                       run_subprocess_mode mode) {
	int fds[2];
	if (pipe(fds) == -1)
		return {};
	pid_t pid = fork();
...
static TSpan<char> read_subprocess(const char *const cmd[], const char *const env[],
                                   int timeoutMs) {
