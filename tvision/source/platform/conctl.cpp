...
void ConsoleCtl::write(const char *data, size_t bytes) const noexcept {
	...
	size_t written = 0;
	int r;
	while (written < bytes && 0 <= (r = ::write(out(), data + written, bytes - written)))
		written += r;
}
...
ConsoleCtl::ConsoleCtl() noexcept {
	...
	if (!isValid(cn[input].handle)) {
		cn[input].handle = CreateFileW(L"CONIN$", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ,
	...
	cn[activeOutput].handle = CreateConsoleScreenBuffer(GENERIC_READ | GENERIC_WRITE, 0, nullptr,
...
ConsoleCtl::~ConsoleCtl() {
	...
	if (activeWindowSize.X != startupWindowSize.X || activeWindowSize.Y != startupWindowSize.Y) {
