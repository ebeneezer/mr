...
NcursesInput::NcursesInput(ConsoleCtl &aCon, NcursesDisplay &, InputState &aState,
                           bool mouse) noexcept
    : InputAdapter(aCon.in()), con(aCon), state(aState), mouseEnabled(mouse) {
...
bool NcursesInput::parseCursesMouse(TEvent &ev) noexcept {
	...
	if (getmouse(&mevent) == OK) {
		...
		for (auto &parseMouse : {TermIO::parseSGRMouse, TermIO::parseX10Mouse}) {
			GetChBuf buf(in);
