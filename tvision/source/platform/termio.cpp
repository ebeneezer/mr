...
static const const_unordered_map<ushort, constarray<ushort, 3>> moddedKeyCodes = {
    ...
    {kbF2, {kbShiftF2, kbCtrlF2, kbAltF2}},
    {kbF3, {kbShiftF3, kbCtrlF3, kbAltF3}},
    {kbF4, {kbShiftF4, kbCtrlF4, kbAltF4}},
    {kbF5, {kbShiftF5, kbCtrlF5, kbAltF5}},
    {kbF6, {kbShiftF6, kbCtrlF6, kbAltF6}},
    {kbF7, {kbShiftF7, kbCtrlF7, kbAltF7}},
    {kbF8, {kbShiftF8, kbCtrlF8, kbAltF8}},
    {kbF9, {kbShiftF9, kbCtrlF9, kbAltF9}},
    {kbF10, {kbShiftF10, kbCtrlF10, kbAltF10}},
    {kbF11, {kbShiftF11, kbCtrlF11, kbAltF11}},
    {kbF12, {kbShiftF12, kbCtrlF12, kbAltF12}},
    {kbEsc, {0, 0, kbAltEsc}},
    {kbBack, {0, kbCtrlBack, kbAltBack}},
    {kbTab, {kbShiftTab, kbCtrlTab, kbAltTab}},
    {kbEnter, {0, kbCtrlEnter, kbAltEnter}},
    {kbHome, {0, kbCtrlHome, kbAltHome}},
    {kbUp, {0, kbCtrlUp, kbAltUp}},
    {kbPgUp, {0, kbCtrlPgUp, kbAltPgUp}},
    {kbLeft, {0, kbCtrlLeft, kbAltLeft}},
    {kbRight, {0, kbCtrlRight, kbAltRight}},
    {kbEnd, {0, kbCtrlEnd, kbAltEnd}},
    {kbDown, {0, kbCtrlDown, kbAltDown}},
    {kbPgDn, {0, kbCtrlPgDn, kbAltPgDn}},
    {kbIns, {kbShiftIns, kbCtrlIns, kbAltIns}},
    {kbDel, {kbShiftDel, kbCtrlDel, kbAltDel}},
};

static ushort getModdedKeyCode(ushort keyCode, ushort tvMods) {
	// Modifier precedece: Shift < Ctrl < Alt.
	int largestMod = (tvMods & kbLeftAlt) ? 2 : (tvMods & kbLeftCtrl) ? 1 : 0;
...
static KeyDownEvent keyWithXTermMods(ushort keyCode, uint mods) noexcept {
	...
	ushort tvMods =
	    (kbShift & -(mods & 1)) | (kbLeftAlt & -(mods & 2)) | (kbLeftCtrl & -(mods & 4));
...
void TermIO::keyModsOff(ConsoleCtl &con) noexcept {
	TStringView seq = far2lDisableSeq "\x1B[?9001l" // Disable win32-input-mode (Conpty).
...
void TermIO::normalizeKey(KeyDownEvent &keyDown) noexcept {
	...
	keyDown.controlKeyState = ((origMods | newMods) & ~(kbCtrlShift | kbAltShift)) |
...
// The sequence ends with 'M' on button press and on 'm' on button release.
{
	...
	int col, row;
	if (!buf.getInt(col) || !buf.getInt(row))
		return Rejected;
	...
	if (csi.length == 1 && terminator == '~') {
		switch (csi.getValue(0)) {
			...
			case 21:
				...
				break;
			case 23:
				ev.keyDown = {{kbShiftF1}, kbShift};
				break;
			case 24:
				ev.keyDown = {{kbShiftF2}, kbShift};
				break;
			case 25:
				ev.keyDown = {{kbShiftF3}, kbShift};
				break;
			case 26:
				ev.keyDown = {{kbShiftF4}, kbShift};
				break;
			case 28:
				ev.keyDown = {{kbShiftF5}, kbShift};
				break;
			case 29:
				ev.keyDown = {{kbShiftF6}, kbShift};
				break;
			case 31:
				ev.keyDown = {{kbShiftF7}, kbShift};
				break;
			case 32:
				ev.keyDown = {{kbShiftF8}, kbShift};
				break;
			case 33:
				ev.keyDown = {{kbShiftF9}, kbShift};
				break;
			case 34:
				ev.keyDown = {{kbShiftF10}, kbShift};
				break;
			case 200:
		...
		if (csi.getValue(0) == 1) {
			...
			return Rejected;
	} else if (csi.length == 3 && csi.getValue(0) == 27 && csi.getSeparator(0) == ';' &&
	           csi.getSeparator(1) == ';' && terminator == '~') {
...
// Konsole, IntelliJ.
{
	...
	if (!keyFromLetter(key, mod, ev.keyDown))
		return Rejected;
	ev.what = evKeyDown;
	return Accepted;
}
...
// http://www.leonerd.org.uk/hacks/fixterms/
{
	...
	//
	// unicode-key-code : shifted-key-code : base-layout-key ; modifiers : event-type ;
...
static ParseResult parseWin32InputModeKey(const CSIData &csi, TEvent &ev,
...
class Win32InputModeUnwrapper : public InputGetter {
  ...
  public:
	...
	int get() noexcept override {
		...
		if (buf.get() == '\x1B' && buf.get() == '[' && csi.readFrom(buf) && csi.terminator == '_' &&
		    parseWin32InputModeKey(csi, ev, state) == Accepted &&
		    ev.keyDown.charScan.scanCode == 0 && ev.keyDown.textLength == 1)
...
ParseResult TermIO::parseWin32InputModeKeyOrEscapeSeq(const CSIData &csi, InputGetter &in,
                                                      TEvent &ev, InputState &state) noexcept
...
bool TermIO::setClipboardText(ConsoleCtl &con, TStringView text, InputState &state) noexcept {
	return setFar2lClipboard(con, text, state) || setOsc52Clipboard(con, text, state);
}

bool TermIO::requestClipboardText(ConsoleCtl &con, void (&accept)(TStringView),
                                  InputState &state) noexcept {
	state.putPaste = &accept;
	return requestFar2lClipboard(con, state) || requestOsc52Clipboard(con, state);
}
...
// response to it. This has to be done after disabling keyboard and mouse extensions.
{
	...
	} while (!state.gotDsrResponse && (steady_clock::now() - begin <= timeout));
}
