...
TButton::TButton(const TRect &bounds, TStringView aTitle, ushort aCommand, ushort aFlags) noexcept
    : TView(bounds), title(newStr(aTitle)), command(aCommand), flags(aFlags),
      amDefault(Boolean((aFlags & bfDefault) != 0)), animationTimer(0) {
...
void TButton::drawTitle(TDrawBuffer &b, int s, int i, TAttrPair cButton, Boolean down) {
	int l, scOff;
