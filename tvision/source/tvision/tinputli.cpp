...
TInputLine::TInputLine(const TRect &bounds, int limit, TValidator *aValid,
                       ushort limitMode) noexcept
    : TView(bounds), maxLen((limitMode == ilMaxBytes) ? min(max(limit - 1, 0), INT_MAX - 1) : 255),
      maxWidth((limitMode == ilMaxWidth) ? limit : INT_MAX),
      maxChars((limitMode == ilMaxChars) ? limit : INT_MAX), curPos(0), firstPos(0), selStart(0),
      selEnd(0), validator(aValid) {
			...
			case evKeyDown:
				...
				switch (event.keyDown.keyCode) {
					...
					case kbIns:
						...
						break;
					default:
						// The event text may contain null characters, but 'data' is
						// null-terminated, so rely on strlen to measure the text length.
						...
						if ((len = strlen(keyText)) > 0) {
							...
							if (checkValid(True)) {
								...
								if (strlen(data) + len <= (uint)maxLen &&
								    ...
								    dataMts.graphemeCount + keyMts.graphemeCount <=
								        (uint)maxChars) {
									if (firstPos > curPos)
										firstPos = curPos;
									memmove(data + curPos + len, data + curPos,
