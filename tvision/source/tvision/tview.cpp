...
TView::TView(const TRect &bounds) noexcept
    : next(0), options(0), eventMask(evMouseDown | evKeyDown | evCommand), state(sfVisible),
      growMode(0), dragMode(dmLimitLoY), helpCtx(hcNoContext), owner(0) {
	setBounds(bounds);
	cursor.x = cursor.y = 0;
...
void TView::moveGrow(TPoint p, TPoint s, TRect &limits, TPoint minSize, TPoint maxSize,
                     uchar mode) {
	TRect r;
...
Boolean TView::focus() {
	...
	if ((state & (sfSelected | sfModal)) == 0) {
		if (owner) {
			result = owner->focus();
			if (result) {
				if (!owner->current || (!(owner->current->options & ofValidate) ||
