...
Boolean drawTree(TOutlineViewer *beingDrawn, TNode *cur, int level, int position, long lines,
                 ushort flags) {
...
char *TOutlineViewer::createGraph(int level, long lines, ushort flags, int levWidth, int endWidth,
...
static Boolean visitNoArg(TOutlineViewer *outLine, TNode *cur, int level, int position, long lines,
...
static TNode *traverseTree(TOutlineViewer *outLine, TOutlineVisitor action, void *arg,
                           int &position, Boolean &checkResult, TNode *cur, int level, long lines,
                           Boolean lastChild) {
...
TNode *TOutlineViewer::iterate(TOutlineVisitor action, void *arg, Boolean checkResult) noexcept {
	int position = -1;
	TNode *root = getRoot();
	if (root)
		return traverseTree(this, action, arg, position, checkResult, root, 0, 0,
		                    Boolean(getNext(root) == 0));
	return 0;
}
...
static Boolean isNode(TOutlineViewer *, TNode *, int, int position, long, ushort) {
...
static Boolean isFocused(TOutlineViewer *focusCheck, TNode *, int level, int position, long lines,
                         ushort flags) {
...
static Boolean countNode(TOutlineViewer *beingCounted, TNode *p, int level, int, long lines,
                         ushort flags) {
	int len;
	char *graph;
...
TOutline::TOutline(const TRect &bounds, TScrollBar *aHScrollBar, TScrollBar *aVScrollBar,
