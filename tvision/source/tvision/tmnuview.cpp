...
TMenuItem::TMenuItem(TStringView aName, TKey aKey, TMenu *aSubMenu, ushort aHelpCtx,
                     TMenuItem *aNext) noexcept {
	name = newStr(aName);
	command = 0;
	disabled = Boolean(!TView::commandEnabled(command));
...
TMenuItem *TMenuView::findItem(TStringView ch) {
	...
	while (p != 0) {
		...
		if (!p->disabled && !shortcut.empty() && TText::equalsIgnoreCase(ch, hotKeyStr(p->name)))
			return p;
		p = p->next;
	}
	return 0;
...
ushort TMenuView::getHelpCtx() {
	...
	while (c != 0 &&
	       (c->current == 0 || c->current->helpCtx == hcNoContext || c->current->name == 0))
		c = c->parentMenu;
