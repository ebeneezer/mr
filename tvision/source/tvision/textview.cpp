...
#pragma argsused
TTextDevice::TTextDevice(const TRect &bounds, TScrollBar *aHScrollBar,
...
TTerminal::TTerminal(const TRect &bounds, TScrollBar *aHScrollBar, TScrollBar *aVScrollBar,
                     ushort aBufSize) noexcept
    : TTextDevice(bounds, aHScrollBar, aVScrollBar), queFront(0), queBack(0) {
...
static void discardPossiblyTruncatedCharsAtEnd(const char (&s)[256], size_t &sLen) {
#if !defined(__BORLANDC__)
	if (sLen == sizeof(s)) {
		sLen = 0;
...
otstream::otstream(TTerminal *tt) : ostream(tt) {
}
