...
TColorSelector::TColorSelector(const TRect &bounds, ColorSel aSelType) noexcept : TView(bounds) {
...
TMonoSelector::TMonoSelector(const TRect &bounds) noexcept
    : TCluster(bounds,
               new TSItem(normal,
                          new TSItem(highlight, new TSItem(underline, new TSItem(inverse, 0))))) {
	eventMask |= evBroadcast;
}
...
TColorDisplay::TColorDisplay(const TRect &bounds, TStringView aText) noexcept
    : TView(bounds), color(0), text(newStr(aText)) {
	eventMask |= evBroadcast;
}
...
TColorGroupList::TColorGroupList(const TRect &bounds, TScrollBar *aScrollBar,
                                 TColorGroup *aGroups) noexcept
    : TListViewer(bounds, 1, 0, aScrollBar), groups(aGroups) {
	int i = 0;
	while (aGroups != 0) {
...
TColorGroupList::TColorGroupList(StreamableInit) noexcept : TListViewer(streamableInit) {
}

#endif

TColorItemList::TColorItemList(const TRect &bounds, TScrollBar *aScrollBar,
                               TColorItem *aItems) noexcept
    : TListViewer(bounds, 1, 0, aScrollBar), items(aItems) {
	eventMask |= evBroadcast;
	int i = 0;
	while (aItems != 0) {
		aItems = aItems->next;
...
TColorItemList::TColorItemList(StreamableInit) noexcept : TListViewer(streamableInit) {
}

#endif

TColorDialog::TColorDialog(TPalette *aPalette, TColorGroup *aGroups) noexcept
    : TWindowInit(&TColorDialog::initFrame), TDialog(TRect(0, 0, 61, 18), colors) {
	options |= ofCentered;
	if (aPalette != 0) {
		pal = new TPalette("", 0);
		*pal = *aPalette;
...
TColorDialog::TColorDialog(StreamableInit) noexcept : TWindowInit(0), TDialog(streamableInit) {
}
