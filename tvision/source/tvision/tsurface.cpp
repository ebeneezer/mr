...
TDrawSurface::TDrawSurface() noexcept : dataLength(0), data(0) {
	size.x = size.y = 0;
}

TDrawSurface::TDrawSurface(TPoint aSize) noexcept : dataLength(0), data(0) {
	resize(aSize);
}
...
TSurfaceView::TSurfaceView(const TRect &bounds, const TDrawSurface _FAR *aSurface) noexcept
    : TView(bounds), surface(aSurface) {
	delta.x = delta.y = 0;
}
