...
Boolean TEditor::insertBuffer(const char *p, uint offset, uint length, Boolean allowUndo,
                              Boolean selectText) {
	selecting = False;
...
void TEditor::write(opstream &os) {
	TView::write(os);
	os << hScrollBar << vScrollBar << indicator << bufSize << (uchar)canUndo
	   << (uchar)lineEndingType << (uchar)encoding;
}
...
void *TEditor::read(ipstream &is) {
	...
	is >> temp;
	lineEndingType = LineEndingType(temp);
	is >> temp;
