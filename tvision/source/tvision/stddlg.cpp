...
TSortedListBox::TSortedListBox(const TRect &bounds, ushort aNumCols,
                               TScrollBar *aScrollBar) noexcept
    : TListBox(bounds, aNumCols, aScrollBar), shiftState(0), searchPos(-1) {
	showCursor();
	setCursor(1, 0);
}
...
TFileInfoPane::TFileInfoPane(const TRect &bounds) noexcept : TView(bounds) {
	eventMask |= evBroadcast;
}
