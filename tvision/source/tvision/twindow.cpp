...
TWindowInit::TWindowInit(TFrame *(*cFrame)(TRect)) noexcept : createFrame(cFrame) {
}

TWindow::TWindow(const TRect &bounds, TStringView aTitle, short aNumber) noexcept
    : TWindowInit(&TWindow::initFrame), TGroup(bounds), flags(wfMove | wfGrow | wfClose | wfZoom),
      zoomRect(getBounds()), number(aNumber), palette(wpBlueWindow), title(newStr(aTitle)) {
	state |= sfShadow;
...
TWindow::TWindow(StreamableInit) noexcept : TWindowInit(0), TGroup(streamableInit) {
}
