...
TKeyInputLine::TKeyInputLine(const TRect &bounds, int aMaxLen) : TInputLine(bounds, aMaxLen) {
}
...
Boolean TNumInputLine::valid(ushort command) {
	...
	if ((command != cmCancel) && (command != cmValid)) {
		...
		if ((value == 0) || (value < min) || (value > max)) {
			select();
			messageBox(mfError | mfOKButton, "Number must be from %ld to %ld.", (long)min,
			           (long)max);
			selectAll(True);
			ok = False;
		}
