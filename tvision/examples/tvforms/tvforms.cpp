...
__link(RResourceCollection) __link(RButton) __link(RCheckBoxes) __link(RInputLine) __link(RLabel)
    __link(RMenuBar) __link(RRadioButtons) __link(RFrame) __link(REditor) __link(RMemo)
...
void TFormApp::openListDialog() {
	...
	d = new TFileDialog(FORM_WILDCARD, "Open File", "~N~ame", fdOpenButton, hlOpenListDlg);
	if (validView(d) != NULL) {
...
TMenuBar *TFormApp::initMenuBar(TRect r) {
	...
	r.b.y = r.a.y + 1;
	return new TMenuBar(
	    r, *new TSubMenu("~\xF0~", hcNoContext) +
...
TStatusLine *TFormApp::initStatusLine(TRect r) {
	...
	return new TStatusLine(
	    r, *new TStatusDef(0, 0xFFFF) + *new TStatusItem("~F2~ Save", kbF2, cmListSave) +
	           *new TStatusItem("~F3~ Open", kbF3, cmListOpen) +
	           *new TStatusItem("~F10~ Menu", kbF10, cmMenu) +
	           *new TStatusItem(0, kbShiftDel, cmCut) + *new TStatusItem(0, kbCtrlIns, cmCopy) +
	           *new TStatusItem(0, kbShiftIns, cmPaste) + *new TStatusItem("", kbCtrlF5, cmResize));
}
