...
__link(RScrollBar) __link(RLabel) __link(RMemo) __link(RButton)
...
TForm *makeForm() {
	const int formX1 = 1, formY1 = 1, formWd = 36, formHt = 17, labelCol = 1, labelWid = 8,
	          inputCol = 11, buttonWd = 12, descrHt = 6;
	...
	y += 3;
	r = TRect(labelCol + descrWidth + 1, y, labelCol + descrWidth + 2, y + descrHt);
	c = new TScrollBar(r);
	f->insert(c);
