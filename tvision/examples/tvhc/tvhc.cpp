...
void copyPath(char *dest, const char *src, size_t size) {
	if (strnzcpy(dest, src, size) < strlen(src)) {
		cerr << "Path too long (" << strlen(src) << " > " << size << "): \"" << src << "\"" << endl;
		exit(1);
	}
}
...
void prntMsg(const char *pref, const char *text) {
	if (lineCount > 0)
		cerr << pref << ": " << textName << "(" << lineCount << "): " << text << "\n";
	else
		cerr << pref << ": " << textName << " " << text << "\n";
}
...
TRefTable::TRefTable(ccIndex aLimit, ccIndex aDelta) : TSortedCollection(aLimit, aDelta) {
}
...
Boolean isEndParagraph(State state) {
	...
	flag =
	    ((line[0] == 0) || (line[0] == commandChar[0]) || (line[0] == '\x1A') ||
	     ((line[0] == ' ') && (state == wrapping)) || ((line[0] != ' ') && (state == notWrapping)));
	if (flag)
		return (True);
	else
		return (False);
...
void _FAR doWriteSymbol(void *p, void *p1) {
	...
	if (((TReference *)p)->resolved) {
		...
		*symbFile << os.str();
		...
		os << "Unresolved forward reference \"" << ((TReference *)p)->topic << "\"" << ends;
		warning(os.str());
	}
}
