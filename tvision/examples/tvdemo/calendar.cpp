...
void TCalendarView::draw() {
	...
	buf.moveChar(0, ' ', color, 22);
	{
		...
		os << setw(9) << monthNames[month] << " " << setw(4) << year << " " << (char)30 << "  "
		   << (char)31 << " " << ends;
	}
...
void TCalendarView::handleEvent(TEvent &event) {
	...
	if (state & sfSelected) {
		if (event.what & (evMouseDown | evMouseAuto)) {
			...
			if (event.keyDown.charScan.charCode == '+' || event.keyDown.keyCode == kbDown) {
				++month;
				if (month > 12) {
					++year;
					month = 1;
				}
			} else if (event.keyDown.charScan.charCode == '-' || event.keyDown.keyCode == kbUp) {
				--month;
				if (month < 1) {
					--year;
					month = 12;
...
TStreamableClass RCalendarWindow(TCalendarWindow::name, TCalendarWindow::build,
