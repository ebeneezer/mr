...
TEventViewer::TEventViewer(const TRect &bounds, ushort aBufSize) noexcept
    : TWindowInit(&initFrame), TWindow(bounds, 0, wnNoNumber) {
...
static void printConstants(ostream &out, ushort value,
                           void(_FAR &doPrint)(ostream _FAR &, ushort)) {
	out << hex << setfill('0') << "0x" << setw(4) << value;
	char buf[256];
...
void TEventViewer::printEvent(ostream &out, const TEvent &ev) {
	...
	if (ev.what & evKeyboard) {
		...
		out << "},\n"
		    << dec << "    .textLength = " << (int)ev.keyDown.textLength << "\n"
		    << "  }\n";
	}
