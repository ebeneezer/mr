...
class QuickMessage : public TWindow {
  ...
  public:
	QuickMessage(const char *drive)
	    : TWindowInit(TWindow::initFrame), TWindow(TRect(15, 8, 65, 19), "Please Wait...", 0) {
...
class TFilePane : public TScroller {
  ...
  public:
	TFilePane(const TRect &bounds, TScrollBar *hsb, TScrollBar *vsb) : TScroller(bounds, hsb, vsb) {
		fileCount = 0;
		files = 0;
	}
	~TFilePane() {
...
class TDirWindow : public TWindow {
  ...
  public:
	TDirWindow(const char *driveInit)
	    : TWindowInit(TWindow::initFrame), TWindow(TRect(1, 1, 76, 21), driveInit, 0) {
...
TMenuBar *TDirApp::initMenuBar(TRect r) {
	...
	return new TMenuBar(r, *new TSubMenu("~\xF0~", kbAltSpace) +
	                           *new TMenuItem("~A~bout...", cmAbout, kbAltA) +
	                           *new TSubMenu("~F~ile", kbAltF) +
	                           *new TMenuItem("~N~ew Window...", cmDirTree, kbAltN) + newLine() +
...
void TDirApp::aboutBox(void) {
	...
	aboutBox->insert(new TButton(TRect(14, 8, 25, 10), " OK", cmOK, bfDefault));
