...
struct LinuxConsoleInput final : public InputAdapter {
	...
	LinuxConsoleInput(ConsoleCtl &aCon, InputAdapter &aInput) noexcept
	    : InputAdapter(aInput.handle), con(aCon), input(aInput) {
	}
...
class LinuxConsoleAdapter final : public ConsoleAdapter {
	...
	LinuxConsoleAdapter(DisplayAdapter &, LinuxConsoleInput &, InputState &, SigwinchHandler *,
	                    GpmInput *) noexcept;
  ...
  public:
	...
	static LinuxConsoleAdapter &create(ConsoleCtl &con, DisplayBuffer &displayBuf,
