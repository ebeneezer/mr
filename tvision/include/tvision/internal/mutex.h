...
struct SignalSafeMutexState {
	...
	struct LockGuard {
		...
		LockGuard(SignalSafeMutexState *aSelf) noexcept : self(aSelf) {
			acquire(self);
		}
...
template <class T> struct SignalSafeReentrantMutex {
	T elem;
	...
	auto lock(Func &&func) noexcept {
		SignalSafeMutexState::LockGuard lk{lockedByCurrentThread() ? nullptr : &state};
		return func(elem);
	}
