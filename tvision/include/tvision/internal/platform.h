...
class DisplayAdapter {
  public:
	...
	virtual TPoint reloadScreenInfo() noexcept {
		return {0, 0};
	}
	...
	virtual TPoint getFontSize() noexcept {
		return {0, 0};
	}

	virtual void writeCell(TPoint /* pos */, TStringView /*text*/, TColorAttr /*attr*/,
...
class InputAdapter : public EventSource {
  public:
	InputAdapter(SysHandle aHandle) noexcept : EventSource(aHandle) {
	}
...
struct ConsoleAdapter {
	...
	ConsoleAdapter(DisplayAdapter &aDisplay, std::vector<EventSource *> &&aSources) noexcept
	    : display(aDisplay), sources(std::move(aSources)) {
	}

	virtual ~ConsoleAdapter() {
	}

	virtual bool isAlive() noexcept {
		return true;
	}
	virtual bool setClipboardText(TStringView) noexcept {
		return false;
	}
	virtual bool requestClipboardText(void (&)(TStringView)) noexcept {
		return false;
	}
};
