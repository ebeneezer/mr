...
class TText {
  public:
	...
	static void scroll(TStringView text, int count, Boolean includeIncomplete, size_t &length,
	...
	static size_t drawStr(TSpan<TScreenCell> cells, size_t indent, TStringView text,
	...
	static Boolean drawOne(TSpan<TScreenCell> cells, size_t &i, TStringView text,
	                       size_t &j) noexcept;
	static Boolean drawOne(TSpan<TScreenCell> cells, size_t &i, TStringView text, size_t &j,
	                       TColorAttr attr) noexcept;
	...
	static size_t drawStrEx(TSpan<TScreenCell> cells, size_t indent, TSpan<const uint32_t> text,
	                        int textIndent, Func &&transformAttr) noexcept;
	static Boolean drawOne(TSpan<TScreenCell> cells, size_t &i, TSpan<const uint32_t> text,
	                       size_t &j) noexcept;
	static Boolean drawOne(TSpan<TScreenCell> cells, size_t &i, TSpan<const uint32_t> text,
	                       size_t &j, TColorAttr) noexcept;
  ...
  private:
	...
	template <class Text> static Lw scrollImplT(Text text, int, Boolean) noexcept;
...
inline void TText::scroll(TStringView text, int count, Boolean, size_t &length, size_t &width) {
...
inline size_t TText::drawStr(TSpan<TScreenCell> cells, size_t indent, TStringView text,
                             int textIndent, TColorAttr attr) {
	// BCC optimization: avoid using min/substr/subspan and use pointer-range loop.
	if (indent < cells.size() && textIndent < text.size()) {
		size_t count = cells.size() - indent;
		if (text.size() < count)
...
inline Boolean TText::drawOne(TSpan<TScreenCell> cells, size_t &i, TStringView text, size_t &j) {
...
inline void TText::scroll(TStringView text, int count, Boolean includeIncomplete, size_t &length,
                          size_t &width) noexcept
...
inline Boolean TText::drawOne(TSpan<TScreenCell> cells, size_t &i, TStringView text,
                              size_t &j) noexcept {
	return drawOneT(cells, i, text, j, [](auto &) {});
}

inline Boolean TText::drawOne(TSpan<TScreenCell> cells, size_t &i, TSpan<const uint32_t> text,
                              size_t &j) noexcept {
	return drawOneT(cells, i, text, j, [](auto &) {});
}
...
template <class Text, class Func>
inline Boolean TText::drawOneT(TSpan<TScreenCell> cells, size_t &i, Text text, size_t &j,
                               Func &&transformAttr) noexcept {
