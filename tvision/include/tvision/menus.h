...
inline TMenuView::TMenuView(const TRect &bounds, TMenu *aMenu, TMenuView *aParent) noexcept
    : TView(bounds), parentMenu(aParent), menu(aMenu), current(0), putClickEventOnExit(True) {
	eventMask |= evBroadcast;
}

inline TMenuView::TMenuView(const TRect &bounds) noexcept
    : TView(bounds), parentMenu(0), menu(0), current(0), putClickEventOnExit(True) {
	eventMask |= evBroadcast;
}
...
inline TStatusItem::TStatusItem(TStringView aText, TKey aKey, ushort cmd,
...
class TStatusDef {
  ...
  public:
	TStatusDef(ushort aMin, ushort aMax, TStatusItem *someItems = 0,
...
inline TStatusDef::TStatusDef(ushort aMin, ushort aMax, TStatusItem *someItems,
