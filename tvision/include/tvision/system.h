...
struct MessageEvent {
	...
	union {
		...
		struct {
			intptr_t : 8 * sizeof(infoPtr) - 32, infoLong : 32;
		};
		struct {
			uintptr_t : 8 * sizeof(infoPtr) - 16, infoWord : 16;
		};
		struct {
			intptr_t : 8 * sizeof(infoPtr) - 16, infoInt : 16;
		};
		struct {
			uintptr_t : 8 * sizeof(infoPtr) - 8, infoByte : 8;
		};
		struct {
			intptr_t : 8 * sizeof(infoPtr) - 8, infoChar : 8;
		};
#endif
	};
};
...
class TDisplay {
  ...
  protected:
	...
	TDisplay(const TDisplay &) noexcept {
		updateIntlChars();
	};
	~TDisplay() {};
