...
inline constexpr TStringView::TStringView() : str(0), len(0) {
}

#if defined(TVISION_STL) && (__cplusplus >= 201703L || __cpp_lib_constexpr_char_traits)
constexpr inline TStringView::TStringView(const char _FAR *str)
    : str(str), len(str ? std::char_traits<char>::length(str) : 0) {
}
#else
inline TStringView::TStringView(const char _FAR *str) : str(str), len(str ? strlen(str) : 0) {
}
#endif

inline constexpr TStringView::TStringView(const char _FAR *str, size_t len) : str(str), len(len) {
}

inline constexpr TStringView::TStringView(TSpan<char> span) : str(span.data()), len(span.size()) {
}

inline constexpr TStringView::TStringView(TSpan<const char> span)
    : str(span.data()), len(span.size()) {
}

#ifdef TVISION_STL
#if __cplusplus >= 201703L || __cpp_lib_string_view
inline constexpr TStringView::TStringView(std::string_view text)
    : str(text.data()), len(text.size()) {
}
...
inline TStringView::TStringView(const std::string &text) : str(text.data()), len(text.size()) {
}
