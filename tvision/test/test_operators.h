...
inline bool operator==(const TEvent &a, const TEvent &b) {
	...
	if (a.what & evKeyboard)
		return a.keyDown.keyCode == b.keyDown.keyCode &&
		       a.keyDown.controlKeyState == b.keyDown.controlKeyState &&
		       a.keyDown.getText() == b.keyDown.getText();
	if (a.what & evMouse)
		return a.mouse.where == b.mouse.where && a.mouse.eventFlags == b.mouse.eventFlags &&
		       a.mouse.controlKeyState == b.mouse.controlKeyState &&
		       a.mouse.buttons == b.mouse.buttons && a.mouse.wheel == b.mouse.wheel;
	if (a.what & evMessage)
		return a.message.command == b.message.command && a.message.infoPtr == b.message.infoPtr;
	abort();
}
...
inline std::ostream &operator<<(std::ostream &os, const TEvent &ev) {
	...
	if (ev.what & evKeyboard) {
		...
		os << ev.message.command << ", " << ev.message.infoPtr;
	}
	os << "}";
	return os;
}
...
inline TEvent keyDownEv(ushort keyCode, ushort controlKeyState, TStringView text = {}) {
	...
	while (ev.keyDown.textLength <= sizeof(ev.keyDown.text) &&
	       ev.keyDown.textLength < text.size()) {
...
inline TEvent mouseEv(TPoint where, ushort eventFlags, ushort controlKeyState, uchar buttons,
                      uchar wheel) {
	TEvent ev{};
	ev.what = evMouse;
