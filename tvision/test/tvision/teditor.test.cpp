...
class DrawableTestGroup : public TGroup {
  public:
	DrawableTestGroup(const TRect &bounds) noexcept : TGroup(bounds) {
	}
	...
	std::vector<std::vector<TStringView>> getBufferText() {
		std::vector<std::vector<TStringView>> text(size.y, std::vector<TStringView>(size.x));
	...
	std::vector<std::vector<int>> getBufferAttributes() {
		std::vector<std::vector<int>> attributes(size.y, std::vector<int>(size.x));
		for (int y = 0; y < size.y; ++y)
			for (int x = 0; x < size.x; ++x)
...
static bool operator==(const TEditorBufferTestOutput &a, const TEditorBufferTestOutput &b) {
	return a.bufferText == b.bufferText && a.lineEndingType == b.lineEndingType;
}

static bool operator==(const TEditorDisplayTestOutput &a, const TEditorDisplayTestOutput &b) {
	return a.text == b.text && a.attributes == b.attributes && a.curPos == b.curPos &&
	       a.delta == b.delta;
}
...
TEST(TEditor, ShouldDetectAndConvertLineEndingsWhenInsertingText) {
	...
	for (const auto &testCase : testCases) {
		...
		memcpy(&editor->buffer[editor->bufSize - initialText.size()], initialText.data(),
		       initialText.size());
...
TEST(TEditor, ShouldDrawTextAndPlaceCursorCorrectly) {
	...
	static const TestCase<TEditorDisplayTestInput, TEditorDisplayTestOutput> testCases[] = {
	    ...
	    // Combining characters should be drawn properly.
	    {
	        {
	            TPoint{2, 1},
	            "a" COMBINING_ZIGZAG_UTF8 "b" COMBINING_ZIGZAG_UTF8 "c" COMBINING_ZIGZAG_UTF8
	            "d" COMBINING_ZIGZAG_UTF8,
	            {},
	            TPoint{1, 0},
	        },
	        {
