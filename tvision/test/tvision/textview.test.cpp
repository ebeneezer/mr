...
static std::ostream &operator<<(std::ostream &os, const TTerminalState &state) {
	os << "{"
	   << "queBack=" << state.queBack << ", queFront=" << state.queFront << ", buffer="
	   << ::testing::PrintToString((const char (&)[sizeof(state.buffer) - 1]) state.buffer) << "}";
	return os;
}

static std::ostream &operator<<(std::ostream &os, const PrevLinesTestInput &input) {
	os << "{" << input.state << ", pos=" << input.pos << ", lines=" << input.lines << "}";
	return os;
}
...
TEST(TTerminal, prevLinesShouldWorkProperly) {
	static constexpr TestCase<PrevLinesTestInput, ushort> testCases[] = {
	    ...
	    {{{0, 4, "a\nb\n"}, 4, 2}, 2},     {{{0, 4, "a\nb\n"}, 4, 3}, 0},
	    {{{0, 4, "a\nb\n"}, 3, 1}, 2},     {{{0, 4, "a\nb\n"}, 3, 2}, 0},
	    {{{0, 4, "a\nb\n"}, 3, 3}, 0},     {{{4, 3, "aaa\0aaaa"}, 3, 1}, 4},
	    {{{4, 3, "a\na\0aaaa"}, 5, 1}, 4}, {{{7, 0, "\0\0\0\0\0\0\0a"}, 0, 1}, 7},
	};
