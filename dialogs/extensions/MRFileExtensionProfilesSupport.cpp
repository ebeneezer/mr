...
bool fileExtensionEditorSettingsDialogRecordToSettings(const FileExtensionEditorSettingsDialogRecord &record, MRFileExtensionEditorSettings &settings, std::string &errorText) {
	...
	}
	{
		...
		if (miniMapWidthText.empty()) {
			errorText = "MINIMAP_WIDTH must be between 2 and 20.";
			return false;
		}
...
[[nodiscard]] bool validateProfileIdLiteral(const EditProfileDraft &draft, std::string &errorText) {
	...
	auto isAllowed = [](unsigned char c) { return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || (c >= '0' && c <= '9') || c == '_' || c == '-' || c == '.'; };
	if (!std::all_of(id.begin(), id.end(), isAllowed)) {
