...
bool promptMultiFileSearchValues(std::string &pattern, MRMultiSearchDialogOptions &options, MultiFileSearchSession &outSession) {
	enum {
...
bool promptMultiFileSarValues(std::string &pattern, std::string &replacement, MRMultiSarDialogOptions &options, MultiFileSearchSession &outSession) {
	enum {
		kFilespecBufferSize = 256,
		kSearchBufferSize = 256,
		kReplacementBufferSize = 256,
...
bool activateSessionCurrentMatch(MultiFileSearchSession &session) {
	...
	editor->setCursorOffset(match->start);
	file->window->applyCommittedBlockState(MREditWindow::bmStream, false, static_cast<uint>(match->start), static_cast<uint>(end));
	editor->revealCursor(True);
...
bool replaceCurrentSessionMatch(MultiFileSearchSession &session, bool advanceAfterReplace, std::string &errorText) {
	...
	if (!editor->replaceRangeAndSelect(static_cast<uint>(start), static_cast<uint>(end), session.replacement.c_str(), static_cast<uint>(session.replacement.size()))) {
	...
	if (!refreshMatchesForSessionFile(session, fileIndex, errorText)) return false;
	if (fileIndex >= session.files.size()) return false;
	...
	}
	{
		...
		while (nextIndex < session.files[fileIndex].matches.size() && session.files[fileIndex].matches[nextIndex].start < nextOffset)
			++nextIndex;
		if (nextIndex >= session.files[fileIndex].matches.size()) nextIndex = session.files[fileIndex].matches.size() - 1;
		session.files[fileIndex].selectedMatchIndex = nextIndex;
		session.selectedFileIndex = fileIndex;
	}
	if (advanceAfterReplace) static_cast<void>(moveSessionMatch(session, 1, false));
	errorText.clear();
	return true;
}
...
MultiFileCollectOutcome collectMultiFileSession(MultiFileSearchSession &session, const MRMultiSearchDialogOptions &options, const std::string &pattern, const std::string &replacement, bool replaceMode, bool keepFilesOpen, std::string &errorText) {
	...
	if (!compileSearchRegex(buildSearchPatternExpression(pattern, textType), !options.caseSensitive, &code, compileError)) {
	...
	for (const MultiFileSearchCandidate &candidate : candidates) {
		...
		if (editor != nullptr && editor->hasPersistentFileName() && normalizedSearchPath(editor->persistentFileName()) == candidate.normalizedPath) {
			text = editor->snapshotText();
			loaded = true;
		} else if (readTextFile(candidate.normalizedPath, text))
			loaded = true;
		if (loaded) static_cast<void>(collectRegexMatches(text, code, matches));
		++filesSearched;
...
bool handleSearchMultiFileSearchReplace() {
	...
	for (;;) {
		...
		if (!promptMultiFileSarValues(pattern, replacement, sarOptions, session)) return true;
