...
#include "router/MRCommandRouterPdf.hpp"
...
class GetLastDialog final : public MRDialogFoundation {
  ...
  private:
	bool handleListWheel(TEvent &event) {
		const std::array<MRColumnListView *, 3> lists{fileList, folderList, workspaceList};

		for (MRColumnListView *list : lists) {
			if (list == nullptr || !list->containsMouse(event)) continue;
			static_cast<void>(list->handleWheel(event));
			clearEvent(event);
			return true;
		}
		clearEvent(event);
