...
[[nodiscard]] std::string quoteShellArgument(const std::string &value);
...
[[nodiscard]] mr::coprocessor::Result runExternalCommandTask(const mr::coprocessor::TaskInfo &info, std::size_t channelId, const std::string &command, const MRBuildHookContext &buildContext = MRBuildHookContext(), const std::string &successAudioUri = std::string(), const std::string &failureAudioUri = std::string(), bool streamOutput = true, bool mergeStandardError = true);
