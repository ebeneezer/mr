...
TSubMenu *createFileMenu() {
	return &(*new TSubMenu("~F~ile", kbAltF) + *new TMenuItem("~O~pen...", cmMrFileOpen, kbF3, hcNoContext, "F3") + *new TMenuItem("~L~oad...", cmMrFileLoad, kbNoKey, hcNoContext) + *new TMenuItem("~S~ave", cmMrFileSave, kbF2, hcNoContext, "F2") + *new TMenuItem("save ~A~s...", cmMrFileSaveAs, kbCtrlF2, hcNoContext, "CtrlF2") + *new TMenuItem("~I~nfo...", cmMrFileInformation, kbNoKey, hcNoContext) + newLine() + *new TMenuItem("~M~erge...", cmMrFileMerge, kbNoKey, hcNoContext) + *new TMenuItem("~P~rint...", cmMrFilePrint, kbNoKey, hcNoContext) + newLine() + *new TMenuItem("~O~S shell", cmMrFileShellToDos, kbAltF9, hcNoContext, "AltF9") + newLine() + *new TMenuItem("E~x~it", cmQuit, kbAltX, hcNoContext, "Alt-X"));
}
...
TSubMenu *createBlockMenu() {
	...
	return &(*new TSubMenu("~B~lock", kbAltB) + *new TMenuItem("~C~opy block", cmMrBlockCopy, kbF8, hcNoContext, "F8") + *new TMenuItem("~M~ove block", cmMrBlockMove, kbShiftF8, hcNoContext, "ShiftF8") + *new TMenuItem("~D~elete block", cmMrBlockDelete, kbCtrlF8, hcNoContext, "CtrlF8") + newLine() + *new TMenuItem("save ~B~lock to disk", cmMrBlockSaveToDisk, kbShiftF2, hcNoContext, "ShiftF2") + *new TMenuItem("~I~ndent block", cmMrBlockIndent, kbAltF3, hcNoContext, "AltF3") + *new TMenuItem("~U~ndent block", cmMrBlockUndent, kbAltF2, hcNoContext, "AltF2") + newLine() + *new TMenuItem("~W~indow copy...", cmMrBlockWindowCopy, kbAltF8, hcNoContext, "AltF8") + *new TMenuItem("w~i~ndow move...", cmMrBlockWindowMove, kbAltF7, hcNoContext, "AltF7") + newLine() + *new TMenuItem("mark ~L~ines of text", cmMrBlockMarkLines, kbF7, hcNoContext, "F7") + *new TMenuItem("mark c~O~lumns of text", cmMrBlockMarkColumns, kbShiftF7, hcNoContext, "ShiftF7") +
	         *new TMenuItem("mark ~S~tream of text", cmMrBlockMarkStream, kbCtrlF7, hcNoContext, "CtrlF7") + *new TMenuItem("~E~nd marking", cmMrBlockEndMarking, kbF7, hcNoContext, "F7") + *new TMenuItem("~t~urn marking off", cmMrBlockTurnMarkingOff, kbCtrlF9, hcNoContext, "CtrlF9") + newLine() + *new TMenuItem(persistentLabel.c_str(), cmMrBlockPersistent, kbNoKey, hcNoContext));
}
...
TSubMenu *createTextMenu() {
	return &(*new TSubMenu("~T~ext", kbAltT) + *new TMenuItem("~L~ayout...", cmMrTextLayout, kbCtrlF3, hcNoContext, "CtrlF3") + *new TMenuItem("p~U~sh position onto marker stack", cmMrSearchPushMarker, kbF4, hcNoContext, "F4") + *new TMenuItem("~G~et position from marker stack", cmMrSearchGetMarker, kbShiftF4, hcNoContext, "ShiftF4") + newLine() + *createUpperCaseItem() + *createLowerCaseItem() + *new TMenuItem("cen~T~er line", cmMrTextCenterLine, kbNoKey, hcNoContext) + *new TMenuItem("time/~D~ate stamp", cmMrTextTimeDateStamp, kbNoKey, hcNoContext) + newLine() + *new TMenuItem("re-~F~ormat paragraph", cmMrTextReformatParagraph, kbAltR, hcNoContext, "AltR"));
}
...
TSubMenu *createOtherMenu() {
	return &(*new TSubMenu("~O~ther", kbAltO) + *createInstallationAndSetupMenuItem() + newLine() + *new TMenuItem("~E~xecute program...", cmMrOtherExecuteProgram, kbF9, hcNoContext, "F9") + *new TMenuItem("~S~top current program", cmMrOtherStopProgram, kbNoKey, hcNoContext) + *new TMenuItem("~R~estart current program", cmMrOtherRestartProgram, kbNoKey, hcNoContext) + *new TMenuItem("~C~lear current output", cmMrOtherClearOutput, kbNoKey, hcNoContext) + *new TMenuItem("find ne~X~t compiler error", cmMrOtherFindNextCompilerError, kbShiftF9, hcNoContext, "ShiftF9") + *new TMenuItem("~M~atch brace or paren", cmMrOtherMatchBraceOrParen, kbCtrlF5, hcNoContext, "CtrlF5") + newLine() + *new TMenuItem("~A~scii table", cmMrOtherAsciiTable, kbAltA, hcNoContext, "AltA") + *new TMenuItem("~E~moji table", cmMrOtherEmojiTable, kbNoKey, hcNoContext));
}

TSubMenu *createMacroMenu() {
	return &(*new TSubMenu("~M~acro", kbAltM) + *new TMenuItem("Macro ~M~anager...", cmMrOtherMacroManager, kbNoKey, hcNoContext) + newLine() + *new TMenuItem("~R~ecording start/stop", cmMrMacroToggleRecording, kbAltF10, hcNoContext, "AltF10"));
}
