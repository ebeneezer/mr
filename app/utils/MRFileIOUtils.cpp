...
std::size_t trimmedTrailingWhitespaceLengthScalar(std::string_view text) noexcept {
    ...
    while (length > 0 &&
           isTrimWhitespaceByte(static_cast<unsigned char>(text[length - 1])))
        --length;
    return length;
}
...
std::size_t trimmedTrailingWhitespaceLengthAvx2(std::string_view text) noexcept {
    ...
    while (length >= 32) {
        ...
        const __m256i isWs =
            _mm256_or_si256(_mm256_or_si256(isSpace, isTab),
                            _mm256_or_si256(isVTab, isFormFeed));
        const std::uint32_t mask =
            static_cast<std::uint32_t>(_mm256_movemask_epi8(isWs));
...
std::size_t trimmedTrailingWhitespaceLengthSse2(std::string_view text) noexcept {
    ...
    while (length >= 16) {
        ...
        const __m128i isWs = _mm_or_si128(_mm_or_si128(isSpace, isTab),
                                          _mm_or_si128(isVTab, isFormFeed));
        const std::uint32_t mask =
            static_cast<std::uint32_t>(_mm_movemask_epi8(isWs));
...
bool containsTabAvx2(std::string_view text) noexcept {
    ...
    for (; i + 32 <= size; i += 32) {
        ...
        if (_mm256_movemask_epi8(_mm256_cmpeq_epi8(bytes, tab)) != 0)
            return true;
    }
    for (; i < size; ++i)
        if (text[i] == '\t')
...
bool containsTabSse2(std::string_view text) noexcept {
    ...
    for (; i + 16 <= size; i += 16) {
        ...
        if (_mm_movemask_epi8(_mm_cmpeq_epi8(bytes, tab)) != 0)
            return true;
    }
    for (; i < size; ++i)
        if (text[i] == '\t')
...
void appendOutputBytes(MRTextSaveStreamState &state, std::string &output, const char *data,
                       std::size_t length) {
...
void appendLineEnding(MRTextSaveStreamState &state, std::string &output,
                      const MRTextSaveOptions &options, bool eofLine) {
...
void appendNormalizedLine(MRTextSaveStreamState &state, std::string &output,
                          std::string_view line, const MRTextSaveOptions &options, bool eofLine) {
    ...
    if (options.truncateTrailingWhitespace) {
        const std::size_t trimmed =
            trimmedTrailingWhitespaceLength(normalizedLine);
...
void emitParsedLine(MRTextSaveStreamState &state, std::string &output,
                    const MRTextSaveOptions &options, bool eofLine) {
    appendNormalizedLine(state, output, state.bufferedLine, options, eofLine);
}

void stageParsedLine(MRTextSaveStreamState &state, std::string &output,
                     const MRTextSaveOptions &options) {
    if (state.hasBufferedLine)
        emitParsedLine(state, output, options, false);
    state.bufferedLine.swap(state.currentLine);
    state.currentLine.clear();
    state.hasBufferedLine = true;
}

void processNormalizedByte(unsigned char byte, MRTextSaveStreamState &state,
                           std::string &output, const MRTextSaveOptions &options) {
...
void finalizeNormalizedTextSaveStream(const MRTextSaveOptions &options,
    ...
    if (!state.hasBufferedLine)
        state.hasBufferedLine = true;
    if (state.hasBufferedLine)
        emitParsedLine(state, output, options, true);
    if (options.eofCtrlZ && (!state.emittedAnyByte || state.lastEmittedByte != kCtrlZ))
        appendOutputByte(state, output, static_cast<char>(kCtrlZ));
}
