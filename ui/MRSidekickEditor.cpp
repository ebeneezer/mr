...
TRect sidekickBoundsFor(MREditWindow *parent, const std::string &text) {
	...
	const int wantedWidth = std::clamp(maxLineLength(lines) + 2, 24, std::max(24, desktop.b.x - desktop.a.x - 2));
...
MRSidekickEditor::MRSidekickEditor(const TRect &bounds, int parentBufferId, std::size_t replaceStart, std::size_t replaceEnd, std::string text, std::string title, std::vector<MRSidekickSpan> placeholders)
    : TView(bounds), mParentBufferId(parentBufferId), mReplaceStart(replaceStart), mReplaceEnd(replaceEnd), mTitle(std::move(title)), mLines(), mPlaceholders(std::move(placeholders)), mPlaceholderIndex(-1), mCursorRow(0), mCursorCol(0) {
...
void MRSidekickEditor::draw() {
	...
	for (int y = 0; y < size.y; ++y) {
		...
		if (y < static_cast<int>(mLines.size())) {
			...
			const int visible = std::min<int>(line.size(), std::max(0, size.x - 2));
			for (int x = 0; x < visible; ++x) {
				const std::size_t offset = lineStartOffset + static_cast<std::size_t>(x);
				buffer.moveChar(static_cast<ushort>(x + 1), line[static_cast<std::size_t>(x)], offsetInPlaceholder(offset) ? highlightColor : textColor, 1);
			}
		}
		if (y == mCursorRow) {
